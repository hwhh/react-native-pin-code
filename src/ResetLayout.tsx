        ...
        <View style={DEFAULT.Styles.reset?.content}>
            {status == PinCodeT.Statuses.Initial && <>
                <Pressable onPress={() => {
                    ...
                    <Text
                        style={[DEFAULT.Styles.reset?.resetButton, styles?.resetButton]}>{textOptions?.resetButton}</Text>
                </Pressable>
            </>}
            {status == PinCodeT.Statuses.ResetPrompted && <>
                ...
                <Pressable onPress={onReset} style={state => ({opacity: state.pressed ? 0.6 : 1})}>
                    <Text
                        style={[DEFAULT.Styles.reset?.resetButton, styles?.resetButton]}>{textOptions?.confirmButton}</Text>
                </Pressable>
            </>}
        </View>
        <View style={styles?.footer}>
            <Text onPress={onCancel}
                  style={[DEFAULT.Styles.reset?.footerText, styles?.footerText]}>{textOptions?.footerText}</Text>
        </View>
    </>
}
